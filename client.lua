	...
	while true do
		...
		for k, v in pairs(Config.Pedlocation) do
			...
			if dist < 40 and pedspawned == false then
				TriggerEvent('fMiner:spawnped',v.Cords,v.h)
				pedspawned = true
			end
			if dist >= 35 then
				pedspawned = false
...
RegisterNetEvent('fMiner:spawnped')
AddEventHandler('fMiner:spawnped',function(coords,heading)
    ...
    SetEntityInvincible(npc, true) --Da ped nebi umrao
...
function startpro()
  QBCore.Functions.Progressbar('fMiner', 'where the gold at...', 19000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
...
RegisterNetEvent('fMiner:mine', function()
  ...
  RequestAnimDict("melee@hatchet@streamed_core")
      ...
      TriggerServerEvent('fMiner:givegold', RandomItem(), RandomNumber())
      Wait(1000)
      TriggerServerEvent('fMiner:givegold', RandomItem(), RandomNumber())
      TaskPlayAnim(PlayerPedId(), "melee@hatchet@streamed_core" ,"plyr_front_takedown", 5.0, -1, -1, 50, 0, false, false, false)
      Wait(4000)
...
RegisterNetEvent('fMiner:openNUI', function()
...
exports['qb-target']:AddBoxZone("ped", vector3(1085.49, -1996.53, 30.99), 1, 1, {
  ...
  options = {
    {
      type = "server",
      event = "fMiner:sellgold",
      icon = 'fas fa-hand-holding-dollar',
      label = 'Sell gold',
    },
    {
      type = "client",
      event = "fMiner:openNUI",
